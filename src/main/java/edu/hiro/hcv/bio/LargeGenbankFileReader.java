...
public class LargeGenbankFileReader
{
	...
	{
		FileHelper.writeFile(NOTFOUND_FILENAME,"");
		FileHelper.writeFile(MULTIPLE_FILENAME,"");
	}	
	...
	*/
	...
	private GenbankSequence multiple(String identifier, List<GenbankSequence> sequences)
	{
    	...
    	FileHelper.appendFile(MULTIPLE_FILENAME,GenbankSequence.createTable(sequences,properties,false).toString());
    	return null;
	}
	*/
	...
	private GenbankSequence notfound(String identifier)
	{
    	...
    	FileHelper.appendFile(NOTFOUND_FILENAME,identifier);
    	return null;
	}
	...
	*/
	...
	public static void splitFile(String filename, String destination)
	{
		...
		try
		{
            ...
            for (String line=reader.readLine(); line!=null; line=reader.readLine())
		    {
		    	...
		    	{
		    		buffer.append(line).append(NEWLINE);
		    		FileHelper.writeFile(destination+locus+".gbk",buffer.toString());
		    	}
	...
	{
		List<String> ids=StringHelper.splitAsList(FileHelper.readFile(idfile),"\n"); //,true BREAK?
		RandomAccessFile reader=null;
		String newfilename=filename+".new";
		FileHelper.writeFile(newfilename,"");
		try
		{
            reader=new RandomAccessFile(new File(filename),"r");
            //String line;
            ...
            for (String line=reader.readLine(); line!=null; line=reader.readLine())
		    {
		    	...
		    	if (!ids.contains(identifier))
		    		FileHelper.appendFile(newfilename,line);
	...
	public static void main(String[] args)
	{
		...
		//if (!table.isEmpty())
		//	FileHelper.writeFile("d:/temp/var-p.falciparum-core.txt",table.toString());
	}
	*/
}
