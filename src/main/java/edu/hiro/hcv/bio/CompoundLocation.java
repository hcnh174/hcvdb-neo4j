...
public class CompoundLocation
{
	...
	public CompoundLocation(String value)
	{
		...
		for (String str : StringHelper.split(value,";"))
		{
			String name="";
			String location=str;
			if (str.indexOf(':')!=-1)
			{
				List<String> parts=StringHelper.splitAsList(str,":");
				name=parts.get(0);
	...
	public List<String> getArchitecture()
	{
		...
		return names;//StringHelper.join(names,";");
	}
	...
	public static class NamedLocation extends SimpleLocation
	{
		...
		{
			this(name);
			for (String sublocation : StringHelper.split(location,","))
			{
	...
	public static class ScoredSubLocation extends SimpleLocation.SubLocation
	{
		...
		public ScoredSubLocation(int start, int end, String comments)
		{
			...
			if (StringHelper.isEmpty(comments))
				return;
		...
		public String toString()
		{
			...
			return super.toString()+"["+this.score+separator+StringHelper.formatScientificNotation(this.evalue,1)+"]";
		}
	}
